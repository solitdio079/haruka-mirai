...
import { useLoaderData, useFetcher, useOutletContext } from 'react-router-dom'
...
export async function loader({ request }) {
    ...
    const date = new Date(url.searchParams.get("date")).toISOString()
    console.log(date)
  try {
    const response = await fetch(
      `https://api.harukamirai.org/orders/filter/${date}?cursor=&limit=5`,
      {
        method: 'GET',
        credentials: 'include',
        headers: {
...
export default function FilterOrders() {
 ...
 const [startDate, setStartDate] = useOutletContext()
  ...
  const [hasMore, setHasMore] = useState(true)
    ...
    if(new Date(Date.now()).getTime() < new Date(date).getTime()) setStartDate(new Date(Date.now()))
    const toastOptions = {
      duration: 5000,
  ...
  const fetchMoreData = async () => {
    try {
      const response = await fetch(
        `https://api.harukamirai.org/orders/filter/${date}?cursor=${cursor}&limit=5`,
        {
          method: 'GET',
          credentials: 'include',
          headers: {
